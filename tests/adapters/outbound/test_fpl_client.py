...
from fpl_data_relay.adapters.outbound.fpl.validation import validate_fpl_model
from fpl_data_relay.domain.fixtures import Fixture
from fpl_data_relay.domain.live import EventStatusResponse
from fpl_data_relay.domain.reference import BootstrapStatic
from fpl_data_relay.domain.rules import model_to_payload
...
def test_upstream_models_silently_ignore_unknown_fields(
    ...
    model = validate_fpl_model(
        model=BootstrapStatic,
        payload=cast("JsonValue", bootstrap_payload(current_ids=[1])),
    )
